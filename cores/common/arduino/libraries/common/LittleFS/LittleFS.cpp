...
FileImplPtr LittleFSImpl::open(const char* path, OpenMode openMode, AccessMode accessMode) {
    ...
    if (rc == LFS_ERR_ISDIR) {
        ...
        LT_IM("LittleFSDirImpl::openFile: rc=%d fd=%p path=`%s` openMode=%d accessMode=%d err=%d\n",
...
int LittleFSImpl::lfs_flash_read(const struct lfs_config *c,
    ...
    return Flash.readBlock(addr, static_cast<uint8_t*>(dst), size) ? 0 : -1;
}
...
int LittleFSImpl::lfs_flash_prog(const struct lfs_config *c,
    ...
    return Flash.writeBlock(addr, static_cast<const uint8_t*>(src), size) ? 0 : -1;
}
